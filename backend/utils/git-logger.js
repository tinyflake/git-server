...
function logGitOperation(operation) {
  try {
    ...
    console.log(`📝 Git操作日志记录: ${operation.type} - ${operation.repository} - ${operation.user?.username || 'anonymous'}`);
...
function getGitLogs(options = {}) {
  try {
    ...
    if (options.user) {
      logs = logs.filter(log => log.user && log.user.username === options.user);
    }
...
function getRepositoryStats(repoName) {
  try {
    ...
    const stats = {
      ...
      uniqueUsers: [...new Set(logs.filter(log => log.user).map(log => log.user.username))],
...
function cleanupOldLogs(daysToKeep = 30) {
  try {
    ...
    console.log(`🧹 清理了 ${logsData.logs.length - filteredLogs.length} 条旧日志`);
    return true;
  } catch (error) {
